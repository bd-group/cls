...
public class ScheduleFactory {
    ...
    public static enum UpdateMode{RealTime,Auto};
    public static enum stdBlockAlgorithm{Greedy,Random};
    public static enum stdStreamAlgorithm{Greedy,Random};
    public static final Map<stdBlockAlgorithm,Class> stdBlockAlgorithmMap = new EnumMap<stdBlockAlgorithm, Class>(stdBlockAlgorithm.class){
	{
	    put(stdBlockAlgorithm.Greedy, BlockAlgorithmGreedy.class);
	    put(stdBlockAlgorithm.Random, BlockAlgorithmRandom.class);
	}
    };
    public static final Map<stdStreamAlgorithm,Class> stdStreamAlgorithmMap = new EnumMap<stdStreamAlgorithm, Class>(stdStreamAlgorithm.class){
	{
    ...
    public static BlockScheduleInterface getBlockScheduleHandler(UpdateMode updateMode,stdBlockAlgorithm algorithm) throws Exception{
	...
	try{
	    ...
	    if(blockSchd != null){
		blockSchd.setAlgorithm((BlockAlgorithmInterface)stdBlockAlgorithmMap.get(algorithm).newInstance());
	    }else{
		blockSchd = UpdateMode.RealTime == updateMode ? new BlockScheduleRealtimeUpdate(algorithm) : new BlockScheduleAutoUpdate(algorithm);
	    }
	}finally{
    ...
    public static BlockScheduleInterface getBlockScheduleHandler(UpdateMode updateMode,BlockAlgorithmInterface algrithm) throws Exception{
	envInfoInitLock.lock();
	try {
	    if(EnvInfoNotInitialized){
		initEnvInfo();
	...
	try{
		...
		blockSchd = UpdateMode.RealTime == updateMode ? new BlockScheduleRealtimeUpdate(algrithm) : new BlockScheduleAutoUpdate(algrithm);
	    }
	}finally{
	    scheduleHandlerLock.unlock();
	    return blockSchd;
    ...
    public static StreamScheduleInterface getStreamScheduleHandler(UpdateMode updateMode,stdStreamAlgorithm algorithm) throws Exception{
	envInfoInitLock.lock();
	try {
	    if(EnvInfoNotInitialized){
		initEnvInfo();
	...
	try{
	    ...
	    if(streamSchd != null){
		streamSchd.setAlgorithm((StreamAlgorithmInterface)stdStreamAlgorithmMap.get(algorithm).newInstance());
	    }else{
		streamSchd = UpdateMode.RealTime == updateMode ? new StreamScheduleRealtimeUpdate(algorithm) : new StreamScheduleAutoUpdate(algorithm);
	    }
	}finally{
	    scheduleHandlerLock.unlock();
	    return streamSchd;
    ...
    public static StreamScheduleInterface getStreamScheduleHandler(UpdateMode updateMode,StreamAlgorithmInterface algorithm) throws Exception{
	envInfoInitLock.lock();
	try {
	    if(EnvInfoNotInitialized){
		initEnvInfo();
	...
	if(UPDATE_INTERVAL_SECOND == -1){
	    throw new Exception("definition updateTntervalSecond is not fond in"+configurationFileName);
	}
